...
test('opens About Lineage from the brand with safe diagnostics and mobile access', async ({ page, context }) => {
  ...
  await expect(about.getByRole('link', { name: /GitHub repository/ })).toHaveAttribute('href', 'https://github.com/lineagehq/lineage');
  await expect(about.getByRole('link', { name: /Documentation/ })).toHaveAttribute('href', 'https://lineagehq.github.io/lineage/docs/');
