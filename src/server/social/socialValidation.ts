...
export function validateSocialWorkItem(project: string, itemId: string): SocialValidationResponse {
  ...
  try {
    for (const variant of item.variants.filter(candidate => candidate.active)) {
      const channel = database.prepare(`select available, disconnected, locked, paused, provider_fingerprint, capability_json from buffer_channels where project_id=? and channel_id=?`)
      ...
      if (variant.revision.alt_text && !variant.revision.alt_text_reviewed) add('alt_text', 'alt_text_review_required', 'Alt text must carry human review provenance.');
    }
